...
**Confirmado nesse apply real** (as pendências que tinham ficado em aberto): `"AzureManagedDomain"` É o nome aceito pelo `domain_management = "AzureManaged"`; `from_sender_domain` É o atributo certo pro `ACS_REMETENTE` (saiu `DoNotReply@<guid>.azurecomm.net`, formato esperado). **Ainda em aberto**: se o papel `"Communication and Email Service Owner"` (único disponível, `dataActions` vazio) realmente autoriza o *envio* de e-mail via Entra ID pela identidade gerenciada da Function — só confirma disparando a Function de verdade (próximo passo, fora do escopo desta sessão de infra).

Infra final em `rg-mvenc-dev`: Storage Account (`stmvencdev<sufixo>`), Function App (`func-mvenc-dev`), ACS/Email, Log Analytics + App Insights, Key Vault, budget e alerta — tudo aplicado, `terraform plan` final sem diferenças (drift zero).
